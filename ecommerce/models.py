...
# Bazaviy model (barcha modellarga qo‘shiladigan umumiy maydonlar)
class BaseModel(models.Model):
    created_at = models.DateTimeField(auto_now_add=True)  # Sana va vaqtni avtomatik kiritish
    updated_at = models.DateTimeField(auto_now=True)
    my_order = models.PositiveIntegerField(default=0)
...
class Product(BaseModel):
    ...
    def get_absolute_url(self):
        ...
        if primary_images.exists():
            return primary_images.first().image.url
        return "products/default_image.jpg"  # default rasmni to'g'rilash
...
def before_save_comment(sender, instance, **kwargs):
    # Maxsus xatoliklarni tekshirish, masalan, ratingni 1-5 oraliqda bo'lishi kerak
